...
router.group(() => {
  ...
  router.get('/channels/invites', 'InvitesController.index') // Returns all pending channel invites of the user
  ...
  router.group(() => {
    router.get('/channels/:channel_id/messages', 'MessagesController.index')

    router.post('/channels/:channel_id/invite/:nickname', 'InvitesController.create') // Invites a user to a channel
    router.post('/channels/invites/:channel_id', 'InvitesController.accept') // Accepts the invite to a channel
    router.post('/channels/:channel_id/notifications', 'ChannelsController.updateNotifStatus') // Changes notification state for a channel: body params => notif_status
    ...
    router.group(() => {
      router.post('/channels/:channel_id/messages', 'MessagesController.store') // Posts a new message to the channel: body params => content, files
      ...
      router.get('/channels/:channel_id/files/:file_id', 'ChannelsController.getFile') // Get a file from the public folder based on its id
